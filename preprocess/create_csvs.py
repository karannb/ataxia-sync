...
def create_ovr_csv(non_overlapping=False):
    '''
    ...
    for video in range(151):
        if not os.path.exists(
                f"data/{'non_overlapping_' if non_overlapping else ''}gait_cycles/{video}"
        ):
            continue
        dict2csv["video_num"].append(video)
        dict2csv["gait_num"].append(
            len(
                os.listdir(
                    f"data/{'non_overlapping_' if non_overlapping else ''}gait_cycles/{video}"
                )))
    ...
    df.to_csv(
        f"data/{'non_overlapping_' if non_overlapping else ''}overall.csv",
        index=False)


def create_gait_csv(non_overlapping=False):
    '''
    Creates a CSV file with the following columns:
    ...
    for _, record in overall.iterrows():
        for gait in range(record["gait_num"]):
            ...
            all_gait["index"].append(
                global_num)  # used while splitting the data
            global_num += 1

    df = pd.DataFrame.from_dict(all_gait)
    df.to_csv(
        f"data/{'non_overlapping_' if non_overlapping else ''}all_gait.csv",
        index=False)
...
if __name__ == "__main__":
    ...
    create_ovr_csv()
    create_gait_csv()
    create_ovr_csv(non_overlapping=True)
    create_gait_csv(non_overlapping=True)
