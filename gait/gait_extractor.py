...
def store_gait_cycles(video:int, peaks:list):
    '''
    ...
    if not os.path.exists(f"data/gait_cycles/{video}"):
        os.makedirs(f"data/gait_cycles/{video}")
    for i, cycle in enumerate(gait_cycles):
        np.save(f"data/gait_cycles/{video}/{i}.npy", cycle)
...
if __name__ == "__main__":
    ...
    for video in range(151):
        ...
        #     continue
        # if "_" in video:
        #     video_num = int(video.split("_")[0])
        peaks = findPeaks(video)
        if len(peaks) <= 2:
            print(f"Video {video}")
        peak_lengths.append(len(peaks))
        gait_lengths, num_gaits = store_gait_cycles(video, peaks)
